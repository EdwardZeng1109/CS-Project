...
public class TetrisTests {
    ...
    void testMakeFastRotations() {
        ...
        while (counter < 4) {
            ...
            for (int a : piece.getLowestYVals()) {
    ...
    void testPlacePiece() {
        ...
        //fill the entire row
        retval = board.placePiece(pieceA, 2, 0);
        board.commit();
        retval = board.placePiece(pieceA, 4, 0);
        board.commit();
        retval = board.placePiece(pieceA, 6, 0);
        board.commit();
    ...
    void testClearRows() {
        ...
        assertEquals(board.getGrid(0, 1), true);
        assertEquals(board.getGrid(1, 0), true);
        assertEquals(board.getGrid(1, 1), true);
        assertEquals(board.getGrid(0, 2), false);
        assertEquals(board.getGrid(2, 0), false);
    }
}
